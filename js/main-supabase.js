// ==========================================
// Supabase Edition - IdeaFlow Mindmap
// ==========================================
// @version 2.0.0
// @description Supabase 통합 마인드맵 애플리케이션
    ...
    if (googleSignInBtn) {
        googleSignInBtn.addEventListener('click', async () => {
            if (!window.SupabaseClient || !window.SupabaseClient.isConfigured()) {
                showToast('⚠️ Supabase 설정을 먼저 완료해주세요.', 'warning', 5000);
                return;
            }
            try {
                await window.SupabaseClient.signInWithGoogle();
                showToast('🔐 Google 로그인 중...', 'info');
            } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Supabase Edition 초기화 시작...');
    ...
    if (window.SupabaseClient && window.SupabaseClient.isConfigured()) {
        ...
        console.warn('⚠️ Supabase가 설정되지 않았습니다. LocalStorage 모드로 작동합니다.');
        showToast('ℹ️ Supabase 설정을 권장합니다. 데모 페이지에서 설정하세요.', 'info', 5000);
    }
...
function initializeAuthStateListener() {
    ...
    window.SupabaseClient.onAuthStateChange(async (event, session) => {
        ...
        if (session?.user) {
            ...
            updateHeaderAuthUI(session.user);
            showToast('✅ ' + session.user.email + ' 로그인 완료!', 'success');
...
function updateHeaderAuthUI(user) {
    const authUI = document.getElementById('headerAuthUI');
    if (!authUI) return;
    ...
    if (user) {
        ...
        authUI.innerHTML = `
            <div class="header-user-info">
                ...
                <button class="btn btn-secondary btn-sm" id="headerSignOutBtn">로그아웃</button>
            </div>
        `;
...
function startLinkAddingMode() {
    ...
    const handleFirstNodeSelection = (e) => {
        ...
        if (clickedNode && state.linkAddingMode && !state.linkFirstNode) {
            ...
            showToast('✅ 첫 번째 노드 선택됨. 연결할 두 번째 노드를 선택하세요', 'success', 4000);
    ...
    const handleSecondNodeSelection = (e) => {
        ...
        if (clickedNode && state.linkAddingMode && state.linkFirstNode) {
            ...
            if (clickedNode === state.linkFirstNode) {
                showToast('⚠️ 같은 노드는 연결할 수 없습니다. 다른 노드를 선택하세요', 'warning', 4000);
                return;
            }
            ...
            if (alreadyConnected) {
                ...
                updateConnections();
                autoSave();
                showToast('🎉 링크가 생성되었습니다!', 'success', 3000);
            }
...
// 자동 저장 (Supabase + LocalStorage 백업)
function autoSave() {
    const data = {
        version: '2.0.0',
    ...
    // Supabase에 저장 시도
    if (currentUser && window.SupabaseClient && window.SupabaseClient.isConfigured()) {
        // 타이머를 사용하여 과도한 저장 방지
        if (autoSaveTimer) {
        ...
        autoSaveTimer = setTimeout(async () => {
            try {
                await window.SupabaseClient.saveMindmap(data);
                console.log('✅ 자동 저장 완료 (Supabase)');
                
                // LocalStorage에도 백업
                localStorage.setItem('mindmap-data', JSON.stringify(data));
            } catch (error) {
                console.error('❌ Supabase 저장 실패, LocalStorage로 백업:', error);
                localStorage.setItem('mindmap-data', JSON.stringify(data));
            }
        }, 1000); // 1초 디바운스
    } else {
...
async function loadFromSupabase() {
    if (!currentUser || !window.SupabaseClient) {
        console.log('ℹ️ 로그인하지 않아 Supabase에서 불러올 수 없습니다.');
        return;
    }
    
    try {
        console.log('📂 Supabase에서 데이터 불러오는 중...');
        const data = await window.SupabaseClient.loadMindmap();
        
        if (data) {
            loadMindmapData(data);
            showToast('✅ 클라우드에서 데이터를 불러왔습니다', 'success');
            console.log('✅ Supabase에서 데이터 불러오기 완료');
        } else {
            console.log('ℹ️ Supabase에 저장된 데이터가 없습니다. LocalStorage 확인...');
            // Supabase에 데이터가 없으면 LocalStorage 확인
            autoLoadFromLocalStorage();
        }
    } catch (error) {
        console.error('❌ Supabase 불러오기 실패:', error);
        showToast('⚠️ 클라우드 불러오기 실패. 로컬 데이터를 사용합니다.', 'warning');
        autoLoadFromLocalStorage();
    }
}
...
function startRealtimeSync() {
    ...
    window.SupabaseClient.subscribeMindmap((payload) => {
